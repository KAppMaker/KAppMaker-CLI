...
**Equalization-based fan-out (1.13.17+)**: Apple's per-territory price grids are not FX-proportional at the same tier number — some markets are far cheaper than FX at a given tier (Kazakhstan, Pakistan, India, Russia, Tanzania, Vietnam), others non-linear the other way (Japan, Korea, Brazil, Indonesia). Earlier versions synthesised per-territory price-point IDs from the USA tier number (1.13.8 added a `LOCAL_PRICE_TERRITORIES` heuristic for seven markets), which under-priced e.g. Kazakhstan ($6.99 base → 240 KZT ≈ $0.46) and over-priced Japan/Korea by 2–4×. KAppMaker now asks Apple directly: for each unique PPP USD target it fetches that USA price point's **equalizations** — Apple's own value-matched price point per territory, the same mapping App Store Connect's UI uses when it fills all countries from a base price. One extra API call per unique target (~6 per base price), cached per run. If your app was set up with an earlier version, re-run `create-appstore-app` or `appstore-monetization-push` — pricing is refreshed idempotently on re-runs.
